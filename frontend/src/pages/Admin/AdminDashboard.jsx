...
export default function AdminDashboard() {
  const [adminSection, setAdminSection] = useState("preapproved"); // 'preapproved' | 'faculty-approvals' | 'academic' | 'users'
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="glass p-6 sm:p-8 rounded-2xl border border-white/10 bg-gradient-to-r from-amber-900/20 via-purple-900/20 to-indigo-900/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-300 text-sm mt-1">
            Pre-verify Student & Faculty IDs, approve faculty registration requests, and assign programming lab subjects.
          </p>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <div className="glass p-3.5 rounded-xl border border-purple-500/30 bg-purple-500/10 space-y-1">
            <Clock className="w-4 h-4 text-purple-300" />
            <div className="text-[10px] text-purple-300 font-semibold uppercase">Pending Faculty</div>
            <div className="text-lg font-bold text-purple-200 font-mono">{stats.pendingFacultyCount || 0}</div>
          </div>

          <div className="glass p-3.5 rounded-xl border border-emerald-500/30 bg-emerald-500/5 space-y-1">
            <ShieldCheck className="w-4 h-4 text-emerald-400" />
            <div className="text-[10px] text-emerald-300 font-semibold uppercase">Pre-Approved List</div>
      ...
      <div className="flex border-b border-white/10 overflow-x-auto space-x-6">
        ...
        >
          <Users className="w-4 h-4" /> User Access Directory
        </button>
      </div>

      {/* TAB 1: PRE-APPROVED REGISTRATION LIST */}
      {adminSection === "preapproved" && <PreApprovedManager />}

      {/* TAB 2: PENDING FACULTY APPROVALS & LAB ASSIGNMENTS */}
      {adminSection === "faculty-approvals" && <FacultyApprovalManager />}

      {/* TAB 3: ACADEMIC STRUCTURE MANAGER */}
      {adminSection === "academic" && <AcademicManager />}

      {/* TAB 4: USER DIRECTORY & LAB MANAGEMENT */}
      {adminSection === "users" && (
          ...
          <div className="p-4 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <select
                ...
                className="bg-slate-900 border border-slate-700 text-white rounded-lg px-3 py-1.5 focus:outline-none"
              >
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-900/80 text-slate-400 uppercase font-mono border-b border-white/10">
                <tr>
                  ...
                  <th className="p-4">Department / Assigned Labs</th>
                  ...
                  <tr key={u._id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4">
                      <select
                        ...
                        className="bg-slate-900 border border-slate-700 text-slate-200 rounded-lg px-2.5 py-1 text-xs font-semibold capitalize focus:outline-none"
                      >
                    ...
                    <td className="p-4 text-slate-300">
                      <div>{u.department?.name || u.department || "General"}</div>
                      {u.role === "faculty" && (
                    ...
                    <td className="p-4 text-right space-x-2">
                      {u.role === "faculty" && (
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 border border-purple-500/30 text-xs font-semibold inline-flex items-center gap-1"
                        >
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="glass p-6 rounded-2xl border border-purple-500/30 bg-slate-950 w-full max-w-xl space-y-4 animate-fade-up">
            ...
            <p className="text-xs text-slate-300">
              Department: <strong className="text-purple-300">{editingFaculty.department?.name || "General"}</strong> &bull; Select labs taught by this faculty:
            </p>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setSelectedLabsForEdit((prev) =>
                        isSelected ? prev.filter((id) => id !== lab._id) : [...prev, lab._id]
                      );
                    }}
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-white/10">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-purple-500 hover:bg-purple-400 text-white font-bold text-xs shadow-lg"
              >

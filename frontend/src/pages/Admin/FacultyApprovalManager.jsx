...
import { UserCheck, CheckCircle2, XCircle, Code2, Building2, IdCard, Mail, ShieldAlert } from "lucide-react";
...
export default function FacultyApprovalManager() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="p-4 rounded-xl glass border border-purple-500/20 bg-purple-500/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-base font-bold text-white flex items-center gap-2">
            <UserCheck className="w-5 h-5 text-purple-400" /> Pending Faculty Approval & Lab Subject Assignment
          </h2>
        ...
        <span className="px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30 text-xs font-mono font-bold shrink-0">
      ...
      {/* Pending List */}
      {faculties.length === 0 ? (
        <div className="glass p-12 rounded-2xl border border-white/10 text-center space-y-2">
        ...
        <div className="grid grid-cols-1 gap-6">
          {faculties.map((fac) => {
            ...
            return (
              <div
                key={fac._id}
                className="glass p-5 rounded-2xl border border-white/10 space-y-4 hover:border-purple-500/40 transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400">
                      ...
                      <span className="flex items-center gap-1">
                        <Building2 className="w-3.5 h-3.5 text-teal-400" /> {fac.department?.name || "Dept N/A"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-3 py-2 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 text-xs font-semibold flex items-center gap-1.5 transition-colors disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      disabled={submittingId === fac._id}
                      className="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs font-bold flex items-center gap-1.5 transition-transform active:scale-95 disabled:opacity-50"
                    >
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-semibold text-slate-300 flex items-center justify-between">
                    <span className="flex items-center gap-1">
                      <Code2 className="w-4 h-4 text-purple-400" /> Select Programming Labs to Assign to this Faculty:
                    </span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                      {deptLabs.map((lab) => {
                        ...
                        return (
                          ...
                          >
                            <div>
                              <div className="font-bold text-white text-xs">{lab.name}</div>
                              <div className="text-[10px] text-slate-400 font-mono">{lab.code}</div>
                            </div>
                            <span className="px-2 py-0.5 rounded bg-purple-500/30 text-purple-300 text-[10px] font-mono font-bold uppercase">
